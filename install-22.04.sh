...
# install-22.04.sh
# Purpose: This script is to run in Tool 3 in Ubuntu 22.04/Python 3.10. 
...
# Ubuntu 22.04 comes with Python 3.10 by default, which is fine for Mininet.
...
# around this by removing any existing egg before reinstalling, then
# falling back to a manual mnexec build/install if `make install` fails.
...
# which does not work on 3.10. Installing with --no-deps and then pinning 
# eventlet/dnspython manually seems to work: 
# eventlet==0.35.2
# dnspython==2.1.0
...
info "[*] Installing Ryu's runtime dependencies"
pip3 install --user \
    netaddr \
    msgpack \
    routes \
...
# Step 5: Mininet test if works
info "Running Mininet connectivity test"
